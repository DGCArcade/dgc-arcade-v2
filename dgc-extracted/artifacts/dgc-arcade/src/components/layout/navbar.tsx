...
import { User, Wallet, LogOut, Menu, Shield } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    <>
      <Link
        ...
        className={`text-sm font-medium uppercase tracking-wider transition-colors ${
          location === "/games" ? "text-primary" : "text-muted-foreground hover:text-foreground"
        }`}
      >
        Games
      </Link>
      <Link
        href="/leaderboard"
        className={`text-sm font-medium uppercase tracking-wider transition-colors ${
          location === "/leaderboard" ? "text-primary" : "text-muted-foreground hover:text-foreground"
        }`}
      >
        Leaderboard
      </Link>
      {isAdmin && (
        <Link
          ...
          className={`text-sm font-medium uppercase tracking-wider transition-colors flex items-center gap-1 ${
            location === "/admin" ? "text-primary" : "text-amber-500/80 hover:text-amber-400"
          }`}
        >
      ...
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-lg bg-primary flex items-center justify-center font-display font-black text-primary-foreground text-xl shadow-[0_0_16px_rgba(255,215,0,0.4)] group-hover:shadow-[0_0_24px_rgba(255,215,0,0.7)] transition-shadow duration-300">
              D
            </div>
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated && user ? (
            <>
              <div className="hidden sm:flex items-center gap-2 bg-secondary/80 rounded-full px-4 py-1.5 border border-primary/20 backdrop-blur-sm">
                ...
                <DropdownMenuContent align="end" className="w-56 bg-card border-border/60 backdrop-blur-xl">
                  <div className="flex items-center justify-start gap-2 p-2">
                    <div className="flex flex-col space-y-1 leading-none">
                      ...
                      <p className="text-xs text-muted-foreground font-mono text-primary sm:hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                className="font-bold uppercase tracking-wider text-sm btn-pulse"
