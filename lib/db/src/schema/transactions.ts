...
transactionsRouter.post("/deposit/initiate", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const plisioCurrency =
      PLISIO_CURRENCY_MAP[currency.toUpperCase()] ?? currency.toUpperCase();
    ...
    const response = await fetch(
      `${PLISIO_API}/invoices/new?${params.toString()}`,
    );
    ...
    req.log.info(
      { plisio_response: JSON.stringify(data) },
      "Plisio raw response",
    );
    if (data.status !== "success" || !data.data) {
      ...
      res
        .status(500)
        .json({
          error: "Payment gateway error: " + (data.message ?? "Unknown error"),
        });
      return;
    }
    ...
    try {
      ...
      const txRes = await fetch(
        `${PLISIO_API}/operations/${data.data.txn_id}?${txParams.toString()}`,
      );
      ...
      req.log.info(
        { tx_response: JSON.stringify(txData).slice(0, 500) },
        "Plisio tx detail response",
      );
      if (txData.status === "success" && txData.data) {
        walletAddress =
          txData.data.wallet_hash ??
          txData.data.source_address ??
          txData.data.address ??
          "";
      ...
      req.log.warn(
        { err: String(e) },
        "Could not fetch Plisio transaction details",
      );
    }
...
transactionsRouter.post("/deposit/callback", async (req, res) => {
  try {
    ...
    const creditAmount = source_amount
    ...
    if (user) {
      ...
      const newTotalDeposited =
        parseFloat(user.totalDeposited ?? "0") + creditAmount;
  ...
  try {
    ...
    if (!user.locationVerified) {
      res
        ...
        .json({
          error:
            "Location verification required before withdrawing. Please enable location access and refresh.",
        });
      return;
    }
    ...
    if (withdrawRatio > 0.9 && accountAgeHours < 2) {
    ...
    const autoDecline =
      fraudScore >= 2 || (withdrawRatio > 0.95 && accountAgeHours < 1);
    ...
    if (autoDecline) {
      ...
      res
        ...
        .json({
          error:
            "Withdrawal declined. Please contact support if you believe this is an error.",
        });
      return;
    }
