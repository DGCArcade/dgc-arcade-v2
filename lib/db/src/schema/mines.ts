...
minesRouter.post("/start", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const existing = await db
      .select()
      .from(minesSessionsTable)
      .where(
        and(
          eq(minesSessionsTable.userId, req.user!.userId),
          eq(minesSessionsTable.status, "active"),
        ),
      )
      .limit(1);
    if (existing.length > 0) {
      res
        .status(400)
        .json({
          error: "Active session exists. Cashout or bust first.",
          sessionId: existing[0].id,
        });
      return;
    }

    const newMinesWagered = parseFloat(user.totalWageredAmount ?? "0") + amount;
    await db
      .update(usersTable)
      .set({
        balance: String(parseFloat(user.balance) - amount),
        totalWageredAmount: String(newMinesWagered),
      })
...
minesRouter.post("/reveal", requireAuth, async (req, res) => {
  ...
  try {
    const [session] = await db
      ...
      .where(
        and(
          eq(minesSessionsTable.id, sessionId),
          eq(minesSessionsTable.userId, req.user!.userId),
        ),
      )
      .limit(1);
    ...
    if (hitMine) {
      ...
      await db
        .update(usersTable)
        .set({ totalBets: user.totalBets + 1 })
        .where(eq(usersTable.id, user.id));
...
minesRouter.post("/cashout", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (revealed.length === 0) {
      res
        .status(400)
        .json({ error: "Reveal at least one cell before cashing out" });
      return;
    }
