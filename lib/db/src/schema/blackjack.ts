  ...
  try {
    ...
    if (existing.length > 0) {
      res
        .status(400)
        .json({
          error: "You have an active hand. Finish it first.",
          handId: existing[0].id,
        });
      return;
    }

    // Deduct bet and track wagered amount
    const newBjWagered = parseFloat(user.totalWageredAmount ?? "0") + amount;
    await db
      .update(usersTable)
      .set({
        balance: String(parseFloat(user.balance) - amount),
        totalWageredAmount: String(newBjWagered),
      })
    ...
    if (status === "player_blackjack") {
      ...
      await db
        .update(usersTable)
        .set({
          balance: String(parseFloat(user.balance) - amount + payout),
...
blackjackRouter.post("/action", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (isComplete) {
      ...
      const newBalance =
        parseFloat(user.balance) - (action === "double" ? bet : 0) + payout;
      await db
        .update(usersTable)
        .set({
      ...
      await db.insert(betsTable).values({
        userId: user.id,
        gameId: hand.gameId,
        amount: String(finalBet),
        payout: String(payout),
        won: payout > 0 && status !== "push",
        multiplier: String(payout / finalBet),
