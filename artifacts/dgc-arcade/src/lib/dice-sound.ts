/**
 * Generate a realistic dice roll sound using Web Audio API
 * Creates a short burst of white noise with envelope to simulate dice rolling
 */
export function generateDiceRollSound(): string {
  ...
  for (let i = 0; i < length; i++) {
    ...
    // Apply exponential decay envelope (starts loud, fades out)
    const progress = i / length;
    const envelope = Math.exp(-progress * 5); // Exponential decay
    data[i] *= envelope;
    
    // Add some frequency modulation by varying amplitude
    const modulation = Math.sin(progress * Math.PI * 8) * 0.3 + 0.7;
    data[i] *= modulation;
  }
...
export function playDiceRollSound() {
  try {
    const audio = new Audio(generateDiceRollSound());
    audio.volume = 0.6;
    audio.play().catch(() => {
