  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-white/5 bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 safe-area-bottom shadow-[0_-8px_30px_rgb(0,0,0,0.5)]">
      <div className="flex h-16 items-center justify-around px-2">
        ...
        {/* Slots — hidden when slotsEnabled is false */}
        {settings.slotsEnabled && (
          <Link href="/slots" className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 ${active("/slots")}`}>
            <Zap className={`w-5 h-5 ${location.startsWith("/slots") ? "animate-pulse" : ""}`} />
            <span className="text-[9px] font-black uppercase tracking-widest">Slots</span>
          </Link>
        )}

        {/* Games — hidden when gamesEnabled is false */}
        ...
        {/* Chicken Road — only shown when gamesEnabled is true; links to /games/chicken-road */}
        {settings.gamesEnabled && (
          <Link
            href="/games/chicken-road"
            className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 ${active("/games/chicken-road")}`}
          >
            <span className={`text-xl ${location === "/games/chicken-road" ? "animate-bounce" : ""}`}>🐔</span>
            <span className="text-[9px] font-black uppercase tracking-widest">Chicken</span>
          </Link>
        )}

        {/* Horse Race — hidden when raceEnabled is false */}
        ...
        {/* Sports — hidden when sportsbookEnabled is false */}
