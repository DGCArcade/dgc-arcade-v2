...
function ChickenRoadGame({ game }: ChickenRoadProps) {
  ...
  return (
    <div className="chicken-road-game-root flex flex-col lg:flex-row gap-4 md:gap-6">
      <style>{`
        .cr-action-btn { transition: all 0.16s cubic-bezier(0.34,1.56,0.64,1); }
        .cr-action-btn:hover:not(:disabled) { transform: scale(1.03); filter: brightness(1.1); }
        .cr-action-btn:active:not(:disabled) { transform: scale(0.97); }
        .cr-action-btn:disabled { opacity: 0.38 !important; cursor: not-allowed !important; }
      `}</style>
      ...
      <div className="chicken-road-bet-panel order-2 lg:order-none lg:w-72 shrink-0 space-y-4 bg-card border border-border rounded-xl p-4">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-1">
            {[
              ...
              { l: "½",   fn: () => setAmount(Math.max(minBet, parseFloat((amount * 0.5).toFixed(2)))) },
              { l: "2×",  fn: () => setAmount(Math.min(maxBet, parseFloat((amount * 2).toFixed(2)))) },
              { l: "MAX", fn: () => setAmount(Math.min(maxBet, parseFloat(String(user?.balance ?? 0)))) },
            ].map(({ l, fn }) => (
              <button key={l} type="button" disabled={!canConfigure} onClick={fn}
                className="text-xs font-bold bg-secondary border border-border rounded py-1 disabled:opacity-40 hover:bg-secondary/80 transition-colors">
                {l}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(Object.keys(STAKE_TIERS) as StakeTier[]).map(t => (
              <button key={t} type="button" disabled={!canConfigure} onClick={() => setTier(t)}
                className={`text-xs font-bold border rounded px-2 py-2 disabled:opacity-40 text-left transition-all ${
                  tier === t ? "border-primary" : "bg-secondary border-border hover:border-primary/50"
                }`}
                style={tier === t ? { background: `${accent}22`, borderColor: accent, color: "#fff" } : {}}>
                <span>{STAKE_TIERS[t].label}</span>
                <div className={`font-mono text-[9px] ${tier === t ? "opacity-80" : "text-muted-foreground"}`}>
                  {STAKE_TIERS[t].desc}
                </div>
              </button>
            ))}
        ...
        {status === "active" && currentLane > 0 && (
          <div className="rounded-lg p-3 border"
            ...
            <div className="text-[10px] text-muted-foreground uppercase">Net Gain ({multiplier.toFixed(2)}×)</div>
        ...
        {/* Action buttons */}
        {isIdle ? (
          <Button className="cr-action-btn w-full font-display font-black uppercase h-12 text-base"
            style={{ background: `linear-gradient(140deg, ${accent}ee, ${accent}aa)`, color: "#000", border: "none", boxShadow: `0 4px 20px ${accent}44` }}
            onClick={startGame} disabled={loading}>
            {loading ? "Starting…" : "Play"}
          </Button>
        ) : status === "active" ? (
          <div className="space-y-2">
            <Button className="cr-action-btn w-full font-display font-black uppercase h-11 text-white"
              style={{ background: "linear-gradient(140deg, #16a34a, #15803d)", border: "none", boxShadow: "0 4px 16px rgba(22,163,74,0.4)" }}
              onClick={cashout} disabled={loading || currentLane === 0}>
              Cashout {multiplier.toFixed(2)}×
            </Button>
            <Button className="cr-action-btn w-full font-display font-black uppercase h-12"
              style={{ background: `linear-gradient(140deg, ${accent}ee, ${accent}aa)`, color: "#000", border: "none", boxShadow: `0 4px 16px ${accent}44` }}
              onClick={crossLane} disabled={loading}>
              {loading ? "Crossing…" : "Go →"}
            </Button>
          </div>
        ) : isEnded ? (
        ...
        {status === "won" && payout > 0 && (
          <div className="rounded-lg p-3 text-center border"
            ...
            <div className="text-2xl font-black font-mono text-green-400">+{formatCurrency(payout)}</div>
          </div>
        )}
        {status === "lost" && (
          <div className="rounded-lg p-3 text-center border"
            ...
            <div className="text-sm font-mono text-red-400/70">-{formatCurrency(amount)}</div>
          </div>
        )}
