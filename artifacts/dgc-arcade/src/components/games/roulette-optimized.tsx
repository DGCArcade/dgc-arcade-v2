/**
 * PERFORMANCE-OPTIMIZED ROULETTE COMPONENT
 * 
 * Key optimizations:
 * 1. React.memo to prevent re-renders from parent changes
 * 2. useCallback for stable function references
 * 3. Memoized derived state (isRed, isBetSelected)
 * 4. Lazy rendering of number picker on low-end devices
 * 5. requestAnimationFrame instead of CSS transitions for SVG rotation
 * 6. Reduced animation frame rate on older hardware
 */

import { useState, useEffect, useRef, useId, useCallback, useMemo, memo } from "react";
...
import { isLowEndDevice, getOptimalFPS } from "@/lib/performance";
...
// Memoized SVG rotation animator
...
// Memoized number picker component
const NumberPicker = memo(({
  ...
  accent: string;
  spinning: boolean;
  isBetSelected: (type: BetType, value: number) => boolean;
  toggleBet: (type: BetType, value: number) => void;
  selectedCount: number;
}) => {
  // Lazy render on low-end devices
  const [showPicker, setShowPicker] = useState(!isLowEnd);
  ...
  if (!showPicker) {
    return <div className="h-32 bg-black/20 rounded-lg animate-pulse" />;
  }
  ...
  return (
    <div>
      <div className="text-xs text-muted-foreground mb-1.5 flex justify-between">
        ...
        {selectedCount > 0 && (
          <span className="text-primary font-bold">{selectedCount} selected</span>
        )}
      </div>
        ...
        {[0, ...Array.from({ length: 36 }, (_, i) => i + 1)].map(num => (
          <button key={num} type="button" onClick={() => toggleBet("number", num)} disabled={spinning}
            className={`h-8 rounded text-xs font-bold transition-all relative ${
              ...
              RED_NUMS.has(num) ? "bg-red-800/80 hover:bg-red-700 text-white" : "bg-zinc-800/80 hover:bg-zinc-700 text-white"
            }`}
            style={isBetSelected("number", num) ? { outline: `2px solid ${accent}`, backgroundColor: RED_NUMS.has(num) ? "#991b1b" : num === 0 ? "#166534" : "#18181b" } : {}}>
            {num}
...
export const Roulette = memo(function RouletteComponent({ game }: RouletteProps) {
  ...
  const optimalFPS = useMemo(() => getOptimalFPS(), []);
  const clipId = useId().replace(/:/g, "");

  const minBet = useMemo(() => parseFloat(String(game.minBet ?? 0.01)), [game.minBet]);
  const maxBet = useMemo(() => parseFloat(String(game.maxBet ?? 1_000_000)), [game.maxBet]);
  ...
  const isRed = useCallback((n: number) => RED_NUMS.has(n), []);
  const isBetSelected = useCallback((type: BetType, value: string|number) => {
  ...
  const toggleBet = useCallback((type: BetType, value: string|number) => {
    if (spinning) return;
    setBets(prev => {
  ...
  }, [bets, amount, user, game.id, requireAuth, toast, placeBet, qc, optimalFPS]);
      ...
      <div className="roulette-bet-panel w-full md:w-80 bg-card border border-border rounded-xl p-6 flex flex-col gap-5">
        ...
        <div className="space-y-3">
          ...
          <NumberPicker
            ...
            toggleBet={toggleBet}
            selectedCount={bets.filter(b => b.betType === "number").length}
          />
        </div>
