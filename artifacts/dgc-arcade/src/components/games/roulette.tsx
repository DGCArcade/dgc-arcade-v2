  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <div className="flex-1 bg-secondary border border-border rounded-xl p-6 flex flex-col items-center justify-center min-h-[440px]">
        <div className="relative">
          ...
          {result !== null && !spinning && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center font-mono font-black text-xl shadow-lg border-2 ${
        ...
        {win !== null && !spinning && (
          <div className={`mt-4 font-display font-black text-2xl uppercase tracking-widest ${win ? "text-green-400" : "text-muted-foreground"}`}>
            {win ? `+${formatCurrency(payout)}` : "No Win"}
          </div>
        )}
        {spinning && <div className="mt-4 text-sm text-muted-foreground font-mono animate-pulse">Spinning…</div>}
      </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mt-2">
            {["MIN","x2","/2","MAX"].map((l,i)=>(
              <Button key={l} variant="outline" size="sm" className="flex-1 text-xs h-7 bg-secondary" disabled={spinning}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{v:"red",l:"Red",col:"bg-red-700"},{v:"black",l:"Black",col:"bg-zinc-800"},{v:"green",l:"Zero",col:"bg-green-700"}].map(b=>(
              <Button key={b.v} variant="outline" size="sm"
                className={`font-bold uppercase text-xs h-10 ${bet.betType==="color"&&bet.betValue===b.v ? "border-primary bg-primary/10":"bg-secondary"}`}
          ...
          <div className="grid grid-cols-2 gap-2">
            {["even","odd"].map(v=>(
              <Button key={v} variant="outline" size="sm"
                className={`font-bold uppercase text-xs h-9 ${bet.betType==="evenodd"&&bet.betValue===v?"border-primary bg-primary/10":"bg-secondary"}`}
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{v:"low",l:"1-18"},{v:"high",l:"19-36"}].map(b=>(
              <Button key={b.v} variant="outline" size="sm"
                className={`font-bold uppercase text-xs h-9 ${bet.betType==="half"&&bet.betValue===b.v?"border-primary bg-primary/10":"bg-secondary"}`}
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-1 max-h-36 overflow-y-auto">
              {[0,...Array.from({length:36},(_,i)=>i+1)].map(num=>(
                <button key={num} onClick={()=>setBet({betType:"number",betValue:num})} disabled={spinning}
